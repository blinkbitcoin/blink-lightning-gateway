...
- **Comments:** keep concise and brief, and only add one when the WHY is non-obvious (a hidden constraint, a subtle invariant, a workaround for a specific bug, behavior that would surprise a reader). Don't restate WHAT the code does — well-named identifiers already do that. Never write multi-paragraph docstrings. Module-level doc comments stay under ~5 lines; per-function comments stay under ~3 lines unless they're documenting a real non-obvious constraint.
- **Test placement:** Pure logic (entity validation, value-object round-trips, mockall-mocked services with no I/O) lives inline as `#[cfg(test)] mod tests` in the source file. **Anything that boots Postgres, opens a TCP connection, or starts a tonic server lives under `tests/` — never inline in `src/`.** Matches workspace practice in `blink-card`, `bria`, `cala`, and `es-entity`. Integration tests run in parallel — each test owns its own testcontainers Postgres (see `tests/integration/common.rs::TestDatabase::new` for the retry-on-Docker-contention pattern lifted from `blink-card/tests/common/mod.rs`); no `#[serial]` markers needed. `rstest` for parameterized cases. Integration tests are a separate compilation unit and can't see the lib's `mockall::automock`-generated mocks (gated on lib `cfg(test)`); hand-write a tiny stub impl of the trait — see `tests/invoice_create_producer_flow.rs::CannedLnd` for the pattern.
- **Test value (don't write tautologies):** Before adding any test, ask: *does this catch a regression that `cargo build` doesn't?* If the answer is "no," skip it. Tautology shapes to spot and reject: programming a mock to return X then asserting it returned X (a recurring trap with mockall's `expect_*().returning(...)` pattern); constructing a struct then asserting its fields equal what you just passed in; testing that a value round-trips through its own constructor. **Useful tests catch:** edge-case data semantics that the type system can't express (`None` vs `Some(0)`, empty Vec vs default), illegal state-machine transitions, the path production *actually* uses when the source code's "main" branch is rare and the "fallback" branch is the common case (canonical example: `src/app/decode.rs::tests::destination_recovered_from_signature` — the `.or_else(invoice.recover_payee_pub_key())` chain looks like a fallback but runs on ~every real BOLT11), error-variant choices on parse failures, and behavioral contracts not enforced by types. When a test's purpose isn't obvious from its name, add a one-line comment explaining the regression it guards — otherwise a "cleanup" review will delete it under the misapprehension that it's redundant.
