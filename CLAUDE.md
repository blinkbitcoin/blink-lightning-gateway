...
- **Tests:** Pure logic (entity validation, value-object round-trips, mockall-mocked services with no I/O) lives inline as `#[cfg(test)] mod tests` in the source file. **Anything that boots Postgres, opens a TCP connection, or starts a tonic server lives under `tests/` — never inline in `src/`.** Matches workspace practice in `blink-card`, `bria`, `cala`, and `es-entity`. `serial_test` for state-bearing, `rstest` for parameterized. Integration tests are a separate compilation unit and can't see the lib's `mockall::automock`-generated mocks (gated on lib `cfg(test)`); hand-write a tiny stub impl of the trait — see `tests/invoice_create_producer_flow.rs::CannedLnd` for the pattern.
