//! Inbound use-cases driven by inbound API. Slice 1a only carries
//! `App::create_invoice` — the GraphQL `lnInvoiceCreate` mutation routes
//! here. Subsequent slices add `send_payment`, `fee_probe`, etc.
...
/// Inbound request shape for `App::create_invoice`. Carries the inputs the
...
impl App {
    ...
    pub async fn create_invoice(&self, request: NewInvoiceRequest) -> Result<Invoice, AppError> {
        ...
        let new_invoice = NewInvoice {
            payment_hash: lnd_resp.payment_hash,
            wallet_id: request.wallet_id,
            amount_msat: request.amount_msat,
            expiry_seconds: request.expiry_seconds,
            memo: request.memo,
            bolt_invoice: lnd_resp.bolt_invoice.clone(),
        };
        ...
        // 3 + 4. Pure entity-create + atomic projection-row + event-row +
        //        outbox-row insert in one transaction.
        let mut tx = self.pool.begin().await?;
        let _id = self
            .invoices
            .persist_in_tx(&mut tx, new_invoice, now)
            .await?;
        ...
        let invoice = self
            .invoices
            .find_by_payment_hash(&lnd_resp.payment_hash)
            .await?;
        Ok(invoice)
    }
...
mod tests {
    ...
    async fn create_invoice_propagates_invoice_error() {
        ...
        bad.expiry_seconds = 30; // below 60s lower bound
